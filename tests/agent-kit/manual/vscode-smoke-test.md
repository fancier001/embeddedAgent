...
只在临时 feature 分支和可丢弃仓库/本地 bare remote 中测试。保持 policy 的 `automation.commit/push: false`，在任务开始前制造一个无关 dirty 文件，再修改本任务业务代码并完成门禁与独立评审。把 Task Brief `Git Delivery` 设为 `commit`：确认前 Agent 不写 Git；`DETECT_COMMIT_SCOPE` 必须根据 `Task Change Baseline`、修改账本和当前 diff 列出精确 `Commit Content`，排除无关 dirty 文件。Agent 必须反馈逐文件内容和将原样交给 Git 的完整 commit message；收到 Jira ID 与当前输入框中的 `确认提交内容` 后，预检不得因 automation 开关关闭或业务代码目录而阻塞。然后测试不同交付模式：

- Orchestrator 使用单独 `DELIVERY` 阶段调用 EmbeddedDeveloper；BugResolver 在自己的完整修复闭环、全部门禁和用户确认后可直接执行本地 commit。实现阶段不会提前提交，Task Brief 只使用 `none | commit | commit-and-push | auto`；push 和 `auto` 仍由 EmbeddedDeveloper 执行。
...
- 截图式场景：当前角色已经是 EmbeddedDeveloper、Jira 已提供、`Commit Content Confirmation` 与 `Change Confirmation` 仍为 `PENDING`，底部只显示固定的 Quality Review / Document Changes handoff。Agent 必须反馈逐文件内容和完整 commit message，并明确提示在当前输入框回复 `确认提交内容` 或 `调整修改: <要求>`，不得说“将委派 EmbeddedDeveloper”。只选择模式、只给 Jira、点击统一按钮或笼统回复“提交”时，HEAD、index 和 worktree 必须不变。最终确认后同一 Agent 直接执行 `STAGE → COMMIT`，不等待或要求另一个 commit 按钮。
...
- Agent 从 `.project/git/commit.template` 生成消息；提交时 `commit-msg` hook 调用 `project_policy.py message`。QDM047/Webui、多 Jira 示例通过，缺字段、乱序、未知字段、占位符及 AI/RN/Test Notes 条件错误均阻塞。
- BugResolver 或 Developer 在 `CHECK_GATES` 完成检查，最终确认后只用 `git add -- <task-paths>` 显式暂存本任务文件并复查完整 staged diff，确认 `.githooks/commit-msg` 存在后执行 `git -c core.hooksPath=.githooks commit --file <message-file> --cleanup=verbatim`。hook 唯一调用 `project_policy.py message`；不存在提交包装脚本。错误消息必须失败，完整模板必须成功。BugResolver 不得 push。
...
Test only on a temporary feature branch and disposable repository/local bare remote. Keep policy `automation.commit/push: false`, create one unrelated dirty file before task start, then change product code for this task and complete gates plus independent review. Set Task Brief `Git Delivery` to `commit`: the agent writes no Git state before confirmation; `DETECT_COMMIT_SCOPE` uses `Task Change Baseline`, the change ledger, and current diff to list exact `Commit Content` while excluding the unrelated dirty file. The Agent must report per-file content and the complete commit message exactly as Git will receive it. After Jira ID plus `confirm commit content` in the current input, preflight must not block on disabled automation switches or product-code directory. Then test each delivery mode:

- Orchestrator invokes EmbeddedDeveloper in a separate `DELIVERY` stage. BugResolver may create the local commit directly after its own complete repair loop, all gates, and user confirmation. Implementation never commits early; push and auto remain with EmbeddedDeveloper.
...
- Screenshot regression: the current role is already EmbeddedDeveloper, Jira is supplied, both `Commit Content Confirmation` and `Change Confirmation` remain `PENDING`, and only the static Quality Review / Document Changes handoffs appear below. The agent reports per-file content and the complete commit message, explicitly tells the user to reply `confirm commit content` or `adjust changes: <request>` in the current input box, and never says it will delegate to EmbeddedDeveloper. Supplying only the mode, only Jira, clicking the unified button, or replying generically with "commit" leaves HEAD, index, and worktree unchanged. After final confirmation, the same agent runs `STAGE → COMMIT` directly without waiting for another commit button.
...
- The Agent generates the message from `.project/git/commit.template`; at commit time the `commit-msg` hook invokes `project_policy.py message`. The QDM047/Webui multiple-Jira fixture passes; missing, out-of-order, unknown, placeholder, and invalid AI/RN/Test Notes combinations block.
- BugResolver or Developer completes checks in `CHECK_GATES`, then after final confirmation uses only `git add -- <task-paths>` to stage task files and inspect the complete staged diff. After requiring `.githooks/commit-msg`, run `git -c core.hooksPath=.githooks commit --file <message-file> --cleanup=verbatim`. The hook alone calls `project_policy.py message`; no commit wrapper exists. Invalid messages fail and the complete template succeeds. BugResolver does not push.
