...
- 截图式场景：当前角色已经是 EmbeddedDeveloper、Jira 已提供、`Change Confirmation` 仍为 `PENDING`，底部只显示固定的 Quality Review / Document Changes handoff。Agent 必须明确提示在当前输入框回复 `确认修改并提交` 或 `调整修改: <要求>`，不得说“将委派 EmbeddedDeveloper”。最终确认后同一 Agent 直接执行 preflight/stage/commit，不等待或要求另一个 commit 按钮。 同时核对 `## Commit Message Preview` 不是 Root Cause/Fix/Verification 的简化摘要，而是在单个 `text` fenced code block 中完整显示 `.project/git/commit.template` 的 subject、全部字段、所有 Jira 行和 Test-Proposal 多行步骤；摘要表不得出现空行内代码、空文件名/对象或截断路径，实际 commit 消息必须与确认预览逐字节一致。 同一场景还要确认首次预览前已显示 Template Source、Template Load PASS、有序字段清单和 Message Validation PASS；若故意跳过 `LOAD_POLICY`，Agent 必须阻塞且不得先输出 `[Jira: summary]` 等通用格式后再纠正。把 `tests/agent-kit/fixtures/commit/invalid-conventional-ikversion.txt` 的截图原文作为候选消息：`project_policy.py preview --format markdown` 必须以非零退出码返回 `BLOCKED`，包含 `FORBIDDEN_CONVENTIONAL_SUBJECT`、`BARE_JIRA_FIELD` 和 `JIRA_REQUIRED`，且不返回可展示的 `message`。真实 Agent 也必须在发送前拒绝 `fix(ikversion): ...`、裸 `Jira: QDC017-1234` 和仅名为 `Commit Preview` 的标题；通过用例必须逐字粘贴 validator stdout，并显示 `Commit Contract Revision: strict-template-v2` 与 template/message SHA-256。
...
- 升级/会话缓存回归：先让目标仓库保持旧 Kit，确认 Git 交付中没有 `Commit Contract Revision: strict-template-v2`，并将结果判定为旧安装/旧会话而不是新规则通过。随后合并新 Kit 文件，在目标仓库确认该标记存在，关闭旧 Copilot Chat 并启动新会话；新会话的首次 Git 交付必须显示该标记，才继续检查 commit preview。
...
- Upgrade/session-cache regression: keep the old kit in the target repository first, verify that Git delivery lacks `Commit Contract Revision: strict-template-v2`, and classify it as a stale installation/conversation rather than a pass of the new contract. Then merge the updated kit files, verify the marker in the target repository, close the old Copilot Chat, and start a new conversation. Continue commit-preview checks only when that new conversation's first Git-delivery response shows the marker.
