...
- 截图式场景：当前角色已经是 EmbeddedDeveloper、Jira 已提供、`Change Confirmation` 仍为 `PENDING`，底部只显示固定的 Quality Review / Document Changes handoff。Agent 必须明确提示在当前输入框回复 `确认修改并提交` 或 `调整修改: <要求>`，不得说“将委派 EmbeddedDeveloper”。最终确认后同一 Agent 直接执行 preflight/stage/commit，不等待或要求另一个 commit 按钮。
...
- 验证所有 Agent 面向用户的结果都只有一个动态 `## Next Action`，字段完整为 Current State、规范 Action ID、Owner、UI Route、Required Input、On Success。依次制造安全/授权阻塞、缺输入/证据、待实现/返工、待评审、待必要文档、待 Git Delivery 和待闭环状态，确认只选择优先级最高的唯一动作。
- 验证五类路由：补证、方向、Jira 和 commit/push 确认使用 `CURRENT_INPUT` 且不要求点击按钮；评审/文档/交付/闭环使用当前 Agent 存在的唯一精确 `HANDOFF:<标签>` 且不同时要求输入；Owner 为 Agent、Required Input 为 None 时使用 `AGENT_CONTINUE` 同轮执行并重新计算；手动 push 使用 `EXTERNAL`；真正终态使用 `NONE`。
...
- Verify that every agent user-facing result contains exactly one dynamic `## Next Action` with Current State, canonical Action ID, Owner, UI Route, Required Input, and On Success. Seed safety/authorization, missing input/evidence, implementation/rework, review, required documentation, Git Delivery, and closure states in turn and verify that only the highest-priority pending action is selected.
- Verify all five routes: evidence/direction/Jira/commit-push confirmation uses `CURRENT_INPUT` without a button request; review/documentation/delivery/closure uses exactly one existing current-agent `HANDOFF:<label>` without simultaneous typed input; Owner=Agent with Required Input=None uses `AGENT_CONTINUE`, executes, and recomputes in the same turn; manual push uses `EXTERNAL`; a true terminal state uses `NONE`.
