...
- 该确认只要求用户主动提供 Jira ID 并确认/修正。Project 从 `.project/project.yml` 或已确认上下文生成；Function block、Summary、`bug fix`、Change Reason、Root Cause、Solution、AI、Affected Function、Applicable Project、RN 和 Test Notes 从真实修改与证据生成。无 Jira、无确认或 Jira 格式错误时返回 `BLOCKED`；不得要求用户逐项填写可生成字段，也不得把缺失标记写入 commit message。
- 截图式场景：当前角色已经是 EmbeddedDeveloper、Jira 已提供、确认表仍为 `PENDING`，底部只显示固定的 Quality Review / Document Changes handoff。Agent 必须明确提示在当前输入框回复 `确认提交`，不得说“将委派 EmbeddedDeveloper”。回复后同一 Agent 直接执行 preflight/stage/commit，不等待或要求另一个 commit 按钮。
...
- 若 `.project` 的 Project 为 `auto` 且上下文无法唯一解析，允许在同一次 Jira 确认中额外询问 Project；除此之外不得扩展为整张 metadata 问卷。AI Agent 参与代码、检查、测试或文档时自动生成 `AI-Tool-Used: Y` 和真实场景/详情。
...
- The confirmation asks only for the user-supplied Jira ID plus confirmation/corrections. Project is derived from `.project/project.yml` or confirmed context. Function block, Summary, `bug fix`, Change Reason, Root Cause, Solution, AI, Affected Function, Applicable Project, RN, and Test Notes come from the actual change and evidence. Missing/invalid Jira or missing confirmation returns `BLOCKED`; never ask the user to fill every derivable field or copy a missing marker into the message.
- Screenshot regression: the current role is already EmbeddedDeveloper, Jira is supplied, confirmation remains `PENDING`, and only the static Quality Review / Document Changes handoffs appear below. The agent explicitly tells the user to reply `confirm commit` in the current input box and never says it will delegate to EmbeddedDeveloper. After that reply, the same agent runs preflight/stage/commit directly without waiting for another commit button.
