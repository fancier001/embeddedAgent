...
description: 四 Agent Kit 配置和最小权限规则 / Four-agent kit configuration and least-privilege rules
---
...
- 只保留 `Orchestrator`、`EmbeddedDeveloper`、`QualityReviewer`、`DocKeeper` 四个 agent。
- 所有 agent 显式设置 `target: vscode`、`user-invocable: true` 和最小工具集；不固定模型，不使用已弃用的 `infer`。
- 只有 `Orchestrator` 可拥有 `agent` 工具和 `agents` allowlist；三个 specialist 不得嵌套委派。
...
- Keep exactly four agents: `Orchestrator`, `EmbeddedDeveloper`, `QualityReviewer`, and `DocKeeper`.
...
- Only `Orchestrator` may have the `agent` tool and an `agents` allowlist; the three specialists must not delegate recursively.
