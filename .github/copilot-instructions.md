...
- Git Delivery handoff 已经把会话切换到 `EmbeddedDeveloper`。预览后输出 `CONFIRM_COMMIT` 并要求用户在当前输入框回复 `确认修改并提交`，或回复 `调整修改: <要求>` 进入 `ADJUST_CHANGESET`；任何调整都使旧确认和受影响门禁失效，必须重新验证、独立评审和生成预览。收到最终确认后由当前 Developer 直接执行 preflight/stage/commit，不得说“将委派 EmbeddedDeveloper”、不得自我委派或等待新的 commit handoff 按钮。`commit-and-push` 在 commit 成功后输出 `CONFIRM_PUSH` 并等待独立的 `确认推送`，收到后才执行 push；`auto` 不增加二次确认。
...
- `Git Delivery: auto` 使用仓库外临时消息文件运行 `git-plan --operation auto --delivery auto`。`OUTPUT_COMMIT_MESSAGE` 不修改 Git且面向用户只输出完整 commit 内容；只有 `AUTO_COMMIT_AND_PUSH` 才能暂存和提交，并在 push 前用首次 fingerprint 与 `--expected-commit` 锁定唯一的新 commit。push 失败保留本地 commit，不回滚或自动重试，输出 `MANUAL_PUSH` 和基于最近一次安全解析 remote/ref 的非 force 命令。
...
- 每个面向用户的结果必须包含且只包含一个动态生成的结构化 `## Next Action`，字段为 Current State、规范 Action ID、Owner、UI Route、Required Input、On Success。按安全/授权、输入/证据、实现/返工、评审、必要文档、Git Delivery、闭环的优先级选择唯一动作；输入使用 `CURRENT_INPUT`，角色切换使用当前 Agent frontmatter 的精确 `HANDOFF:<标签>`，已授权的 Agent 动作用 `AGENT_CONTINUE` 同轮执行并重新计算，外部动作/终态使用 `EXTERNAL`/`NONE`。所有 commit 模式进入确认前必须有 Documentation=`PASS` 或带理由的 `NOT_RUN`。
...
- The Git Delivery handoff has already switched the conversation to `EmbeddedDeveloper`. After the preview, emit `CONFIRM_COMMIT` and tell the user to reply `confirm changes and commit`, or `adjust changes: <request>` to enter `ADJUST_CHANGESET`, in the current input box. Every adjustment invalidates old confirmation and affected gates, so rerun verification, independent review, and the preview. On final confirmation, the current Developer runs preflight/stage/commit directly; never say it will delegate to EmbeddedDeveloper, delegate to itself, or wait for another commit handoff button. For `commit-and-push`, emit `CONFIRM_PUSH` after commit and wait for a separate `confirm push` before pushing; `auto` adds no second confirmation.
...
- Every user-facing result contains exactly one dynamically generated structured `## Next Action` with Current State, canonical Action ID, Owner, UI Route, Required Input, and On Success. Select the sole action by safety/authorization, input/evidence, implementation/rework, review, required documentation, Git Delivery, then closure priority. Use `CURRENT_INPUT` for typed input, the exact current-frontmatter `HANDOFF:<label>` for role changes, `AGENT_CONTINUE` for authorized same-turn work followed by recomputation, and `EXTERNAL`/`NONE` for external/terminal states. Every commit mode requires Documentation=`PASS` or a justified `NOT_RUN` before confirmation.
