...
输出唯一完整动态 `Next Action`，包含规范 Action、`UI Route`、`Dispatch Target`、Required Input、`Instruction` 和 On Success。角色切换使用统一按钮路由；已授权且无需输入的动作同轮执行；两轮后仍有重大问题时使用 `FAILED`。
...
Provide exactly one complete dynamic `Next Action` with canonical Action, `UI Route`, `Dispatch Target`, Required Input, `Instruction`, and On Success. Route role transitions through the unified button, continue authorized no-input work in the same turn, and use `FAILED` when major issues remain after two rounds.
