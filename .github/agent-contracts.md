...
- `POLICY LOAD EVIDENCE GATE`：`LOAD_POLICY` 是 `SYNTHESIZE_METADATA`、`CONFIRM_DELIVERY` 和任何 commit preview 的不可跳过前置条件。完成状态必须证明已从 `.project/project.yml` 解析 Git policy，读取实际 `.project/git/delivery.yml` 和它引用的 `.project/git/commit.template`，并记录 `Template Source`、`Template Load: PASS` 及模板的有序字段清单。缺文件、读取失败、字段缺失/乱序或无法验证时返回 `BLOCKED`；禁止依据常见约定、先前任务、模型记忆或 `[Jira: summary]` 等通用格式生成备用消息。有效 Jira 到位后，展示预览前还必须运行 `project_policy.py message`，记录命令、退出码和 `Message Validation: PASS`；否则不得进入 `CONFIRM_DELIVERY`。
...
- `STRICT COMMIT OUTPUT SHAPE GATE`：启用 `.project` 的仓库使用 `Commit Contract Revision: strict-template-v2`。Agent 必须把完整消息写入仓库外临时文件，运行 `python .github/agent-kit/scripts/project_policy.py preview --root <root> --file <temp-file> --format markdown`，且只在退出码 0、状态 `PASS` 时逐字粘贴该命令的完整 stdout。stdout 中的 `Template Source`、`Template SHA-256`、`Message SHA-256`、`## Commit Message Preview` 和 fenced message 均不得重建、删减、改名或改写；同一临时文件的原始字节才可交给 `git commit`。Conventional Commit subject（例如 `fix(ikversion): ...`）、裸 `Jira:`、仅名为 `Commit Preview` 的标题、非 `^<[^<>]+><[^<>]+>:\\s+\\S` subject 或缺少 `<Jira ID>:` 的消息必须在发送前阻塞，不得先显示错误格式再纠正。
- `COMMIT PREVIEW COMPLETENESS GATE`：有效 Jira 已存在时，`Commit Content` 后必须输出 `## Commit Message Preview`，并在单个 `text` fenced code block 中逐字展示由 `.project/git/commit.template` 生成且已经 `project_policy.py preview --format markdown` 校验并渲染的完整提交消息。预览必须依次包含 subject、`<Change Type>`、`<Change Reason>`、`<Root Cause>`、`<Solution>`、全部 `<Jira ID>`、`<AI-Tool-Used>`、`<AI-Tool-Scenario>`、`<AI-Tool-Detail>`、`<Affected Function Name>`、`<Applicable Project>`、`<RN>`、`<RN description>`、`<<<Test Notes>>>`、`<Test-Proposal>`、`<Stress-Test>` 和 `<HW-Test>`，包括 Test-Proposal 的多行步骤；不得用 `Generated Metadata`、Root Cause/Fix/Verification 摘要、表格、`...` 或省略号替代。最终确认后传给 `git commit` 的消息必须与该预览逐字节一致。Jira 尚缺失时将预览明确标记为 `BLOCKED — Jira ID required`，先请求 Jira，再重新生成完整预览并请求内容确认，不得伪造看似完整的消息。
- `Commit Delivery Confirmation` 的 Summary、Root Cause、Fix、Files Changed 和 Tests 必须各自完整、可独立理解，并准确命名 diff 中的文件、符号和修改对象。任何空行内代码片段（例如两个反引号之间没有内容）、空路径/对象、截断路径、缺少模板行或依赖下方内容补全含义的摘要都使整份预览无效；发送前必须丢弃并从证据重新渲染。
...
- `Git Delivery: auto` 必须在任何自动暂存或 commit 前单独满足 Commit Content 确认门禁：预览显示 `Commit Content Confirmation: PENDING`，唯一 `Next Action` 为 `CONFIRM_COMMIT + CURRENT_INPUT + NONE`，`Instruction` 给出包含 `确认自动提交内容`、当前完整 fingerprint、Jira ID 和 `Git Delivery: auto` 的可复制格式。选择 auto、automation 开关、`执行下一步 / Next Action` 点击或先前的模式授权都不构成内容确认。用户确认后，`AUTO_DECIDE` 必须把该 fingerprint 作为 `--expected-content-fingerprint` 传入只读预检；缺失或漂移返回 `CONFIRM_COMMIT_CONTENT` 并重新生成预览。只有 `content_confirmation.status: CONFIRMED` 且当前 fingerprint 匹配时才能进入 `STAGE → COMMIT`。
...
- Commit Message Preview: <Jira 有效时，在单个 text fenced code block 中展示完整且已校验的模板消息；否则写 BLOCKED — Jira ID required>
- Change Confirmation: PENDING
- Reply With: 普通模式使用 `<Jira ID> + 确认修改并提交`；auto 使用 `Git Delivery: auto；Jira ID: <ID>；确认自动提交内容；fingerprint: <完整值>`；或 `调整修改: <移除文件/缩小 hunk/减少实现>`
```
...
- `project_policy.py git-plan --operation auto --delivery auto` 只读返回 `CONFIRM_COMMIT_CONTENT`、`AUTO_COMMIT_AND_PUSH`、`OUTPUT_COMMIT_MESSAGE` 或无有效 diff 时的 `NO_DELIVERY`。存在变更时返回 `commit_content` 和 `content_confirmation`；只有显式传入与当前内容相同的 `--expected-content-fingerprint` 才能把确认状态置为 `CONFIRMED`。消息文件必须位于仓库外的操作系统临时目录，不能成为工作树变更。
...
- `POLICY LOAD EVIDENCE GATE`: `LOAD_POLICY` is a non-skippable precondition for `SYNTHESIZE_METADATA`, `CONFIRM_DELIVERY`, and any commit preview. Completion proves that `.project/project.yml` resolved the Git policy, the actual `.project/git/delivery.yml` and referenced `.project/git/commit.template` were read, and `Template Source`, `Template Load: PASS`, and the ordered template field list were recorded. A missing file, read failure, missing/out-of-order field, or failed validation returns `BLOCKED`; never fall back to common conventions, a previous task, model memory, or a generic `[Jira: summary]` format. After valid Jira is available and before showing a preview, run `project_policy.py message` and record the command, exit code, and `Message Validation: PASS`; otherwise `CONFIRM_DELIVERY` is unreachable.
...
- `STRICT COMMIT OUTPUT SHAPE GATE`: repositories with `.project` use `Commit Contract Revision: strict-template-v2`. The Agent writes the completed message to a repository-external temporary file and runs `python .github/agent-kit/scripts/project_policy.py preview --root <root> --file <temp-file> --format markdown`. Only exit code 0 with `PASS` may be displayed, by pasting the command's complete stdout verbatim. Its `Template Source`, `Template SHA-256` and `Message SHA-256` values, `## Commit Message Preview`, and fenced message must never be reconstructed, shortened, renamed, or rewritten; only the same temporary-file bytes may reach `git commit`. Block before sending any Conventional Commit subject such as `fix(ikversion): ...`, bare `Jira:`, a heading named only `Commit Preview`, a subject outside `^<[^<>]+><[^<>]+>:\\s+\\S`, or a message without `<Jira ID>:`; never display the bad shape and correct it later.
- `COMMIT PREVIEW COMPLETENESS GATE`: when a valid Jira ID is available, follow `Commit Content` with `## Commit Message Preview` and show the complete message generated from `.project/git/commit.template`, validated, and rendered by `project_policy.py preview --format markdown` verbatim in a single `text` fenced code block. The preview contains, in order, the subject, `<Change Type>`, `<Change Reason>`, `<Root Cause>`, `<Solution>`, every `<Jira ID>`, `<AI-Tool-Used>`, `<AI-Tool-Scenario>`, `<AI-Tool-Detail>`, `<Affected Function Name>`, `<Applicable Project>`, `<RN>`, `<RN description>`, `<<<Test Notes>>>`, `<Test-Proposal>`, `<Stress-Test>`, and `<HW-Test>`, including multiline Test-Proposal steps. Never replace it with `Generated Metadata`, a Root Cause/Fix/Verification synopsis, a table, `...`, or an ellipsis. After final confirmation, the message passed to `git commit` is byte-for-byte identical to this preview. If Jira is missing, mark the preview `BLOCKED - Jira ID required`, request Jira first, then regenerate the full preview and request content confirmation; never fabricate an apparently complete message.
- Every Summary, Root Cause, Fix, Files Changed, and Tests value in `Commit Delivery Confirmation` is complete and independently understandable, naming the actual files, symbols, and changed objects from the diff. An empty inline-code span, empty path/object, truncated path, missing template line, or summary that relies on later content to restore omitted meaning invalidates the whole preview; discard and rerender it from evidence before sending.
...
- Commit Message Preview: <when Jira is valid, show the complete validated template message in one text fence; otherwise write BLOCKED - Jira ID required>
- Change Confirmation: PENDING
