...
- `DELIVERY`：仅在修复、测试、必需检查、独立质量评审和必要文档全部为 `PASS` 后进入。显式 `Git Delivery: none` 时记录跳过并关闭；若交付选择仍为 pending，先用 `Task Change Baseline`、`Task Change Ledger` 和当前 diff 执行 `DETECT_COMMIT_SCOPE`，在 `Commit Delivery Confirmation` 中逐文件列出 Git state、增删统计、真实摘要、排除路径和 fingerprint，并标记 `Change Confirmation: PENDING`；建议 `commit` 作为待确认默认值，但确认前不构成授权。Jira ID 必须由用户主动提供；其余 commit metadata 由证据生成。用户要求调整时走 `ADJUST_CHANGESET` 并重新验证。`commit-and-push`/`auto` 必须显式选择；选择 auto 仍不确认内容，预览还必须显示 `Commit Content Confirmation: PENDING` 并要求用户用当前 fingerprint `确认自动提交内容`。确认后三种模式使用单独完整 Task Brief 调用 `EmbeddedDeveloper`；`commit-and-push` 在 commit 后等待 `CONFIRM_PUSH`，`auto` 只有在 `content_confirmation.status: CONFIRMED` 后才进入自动 commit/push。BugResolver 自身不得执行 Git 写操作。 Jira 已存在时，预览还必须完整显示由仓库模板生成并校验的 `## Commit Message Preview`；不得省略任一模板行、Test-Proposal 步骤或真实修改对象，也不得出现空行内代码。
...
- `DELIVERY`: enter only after the repair, tests, required checks, independent quality review, and required documentation are all `PASS`. Record and close explicit `none`. Use the baseline, ledger, and actual diff to show exact per-file Commit Content and `Change Confirmation: PENDING`; Jira is user-supplied and other metadata comes from evidence. Adjustments enter `ADJUST_CHANGESET` and invalidate confirmation. `commit-and-push`/`auto` require an explicit choice, but choosing auto still does not confirm content: the preview also shows `Commit Content Confirmation: PENDING` and requires `confirm automatic commit content` with the current fingerprint. Invoke `EmbeddedDeveloper` only with the confirmed delivery brief; `commit-and-push` waits for `CONFIRM_PUSH`, while auto may commit/push only after `content_confirmation.status: CONFIRMED`. BugResolver never performs Git writes. When Jira is already available, the preview also includes a complete, validated `## Commit Message Preview` generated from the repository template, with no omitted template line, Test-Proposal step, actual changed object, or empty inline-code span.
