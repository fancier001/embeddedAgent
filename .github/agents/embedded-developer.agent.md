...
- `CHECK_GATES`：执行 policy 中对应的全部检查并核对独立评审已通过；对 `commit`、`commit-and-push` 和 `auto` 统一核对 Documentation 为 `PASS`，或为带 `Not required: <reason>` 的 `NOT_RUN`。Documentation 缺失、失败或理由不充分时停止，并动态输出 `Action: DOCUMENT_CHANGES`、`UI Route: NEXT_ACTION_BUTTON`、`Dispatch Target: HANDOFF:文档同步 / Document Changes`；不得进入 `DETECT_COMMIT_SCOPE`。任一其他门禁失败也立即停止。
...
- `CONFIRM_DELIVERY`：一次性展示 `Commit Delivery Confirmation`，其中 `Commit Content` 表逐文件列出是否纳入、Git state、增删统计和真实变更摘要，并展示 `excluded_paths`、fingerprint、建议/已选模式、完整 metadata 预览及 `Change Confirmation: PENDING`。`Git Delivery: auto` 还必须显式显示 `Commit Content Confirmation: PENDING`；选择 auto、启用 automation 或得到 `AUTO_COMMIT_AND_PUSH` 都不构成内容确认。auto 的可复制回复必须包含 `确认自动提交内容`、当前完整 fingerprint、Jira ID 和 `Git Delivery: auto`。其他模式请求用户提供 Jira ID 后回复 `确认修改并提交`。用户也可回复 `调整修改: <移除文件/缩小 hunk/减少实现>`/列出 metadata 修正。预览末尾动态输出 `Action: CONFIRM_COMMIT`、`Owner: User`、`UI Route: CURRENT_INPUT`、`Dispatch Target: NONE` 和精确 `Instruction`。统一按钮点击不构成确认。用户最终确认后，当前 EmbeddedDeveloper 直接继续，不得自我委派或等待另一个按钮。预检重新运行相同路径的 `git-plan` 并比较 fingerprint；路径或内容漂移时确认失效并返回预览。用户未确认时返回 `BLOCKED` 且不执行 Git 写入。
...
- `CONFIRM_PUSH`：仅用于 `commit-and-push`。输出 commit SHA、branch、remote alias、脱敏 URL、目标 ref、`Action: CONFIRM_PUSH`、`UI Route: CURRENT_INPUT`、`Dispatch Target: NONE` 和回复 `确认推送` 的精确 `Instruction`。统一按钮点击不构成 push 确认；确认后由当前 Developer 直接继续，不重复 Jira、metadata 或 commit 确认。
...
报告必须包含共享契约定义的唯一完整 `## Next Action`。输入确认使用 `CURRENT_INPUT + NONE` 并给出可复制 `Instruction`；独立评审、文档和闭环使用 `NEXT_ACTION_BUTTON` 与当前 Agent 的精确基础按钮 Dispatch Target；手动 push 使用 `EXTERNAL + NONE`；终态使用 `NONE + NONE`。已授权且无需输入的 Agent 动作同轮执行；统一按钮只负责安全路由，不能替代 commit/push 确认。

本 Agent 的 Dispatch Target 固定映射为：`QUALITY_REVIEW → HANDOFF:独立评审 / Quality Review`、`DOCUMENT_CHANGES → HANDOFF:文档同步 / Document Changes`、`CLOSE_ISSUE → HANDOFF:问题已解决 / Close Issue`。这些基础按钮始终可见且仅作人工备用入口；动态默认入口是末尾的 `执行下一步 / Next Action`。提前点击不匹配的基础按钮时返回 `BLOCKED`，不修改、commit 或 push。
...
- `CONFIRM_DELIVERY`: show one complete confirmation preview and end it with `Action: CONFIRM_COMMIT`, `Owner: User`, `UI Route: CURRENT_INPUT`, `Dispatch Target: NONE`, and a copy-ready `Instruction` for Jira/change confirmation or adjustment. For `Git Delivery: auto`, also show `Commit Content Confirmation: PENDING`; selecting auto, enabling automation, or receiving `AUTO_COMMIT_AND_PUSH` is not content confirmation. The copy-ready auto reply includes `confirm automatic commit content`, the current full fingerprint, Jira ID, and `Git Delivery: auto`. Clicking the unified button is not confirmation. Drift invalidates confirmation and regenerates the preview. Without explicit confirmation, return `BLOCKED` and perform no Git write.
...
- `CONFIRM_PUSH`: use only for `commit-and-push`. Emit the commit SHA, branch, remote alias, redacted URL, target ref, `Action: CONFIRM_PUSH`, `UI Route: CURRENT_INPUT`, `Dispatch Target: NONE`, and an exact `confirm push` Instruction. The unified-button click is not push confirmation. Never push before explicit input.
...
- `PUSH`: use only the policy-approved non-force command. If an automatic push fails, keep the local commit without rollback or retry and emit `Action: MANUAL_PUSH`, `UI Route: EXTERNAL`, and `Dispatch Target: NONE`; `Instruction` includes the working directory, safely resolved command, expected result, and evidence to return.
...
The report includes exactly one complete shared-contract `## Next Action`. Typed confirmation uses `CURRENT_INPUT + NONE` with a copy-ready Instruction; review, documentation, and closure use `NEXT_ACTION_BUTTON` with the exact base-button Dispatch Target; manual push uses `EXTERNAL + NONE`; and a terminal state uses `NONE + NONE`. Continue authorized no-input work in the same turn. The unified button routes safely but never replaces commit or push confirmation.

This agent's Dispatch Target mapping is fixed: `QUALITY_REVIEW → HANDOFF:独立评审 / Quality Review`, `DOCUMENT_CHANGES → HANDOFF:文档同步 / Document Changes`, and `CLOSE_ISSUE → HANDOFF:问题已解决 / Close Issue`. Base buttons remain visible as manual fallbacks; the final unified button is the dynamic default. An early mismatched base-button click returns `BLOCKED` without edits, commits, or pushes.
