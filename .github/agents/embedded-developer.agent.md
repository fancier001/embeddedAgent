...
- `CONFIRM_DELIVERY`：一次性展示 `Commit Delivery Confirmation`，其中 `Commit Content` 表逐文件列出是否纳入、Git state、增删统计和真实变更摘要，并展示 `excluded_paths`、fingerprint、建议/已选模式、完整 metadata 预览及 `Change Confirmation: PENDING`。先展示 `Template Source`、`Template Load: PASS`、有序字段清单，以及 `project_policy.py message` 的完整命令、退出码和 `Message Validation: PASS`；任一项缺失即禁止确认。当 Jira 有效时，必须紧接着输出 `## Commit Message Preview`，在单个 `text` fenced code block 中逐字展示 `.project/git/commit.template` 的全部有序行以及 Test-Proposal 多行步骤，并先通过 `project_policy.py message`；不得用 Root Cause/Fix/Verification 摘要、`Generated Metadata`、表格或省略号代替。最终 commit 消息必须与该预览逐字节一致。Jira 缺失时明确显示 `BLOCKED — Jira ID required`，收到 Jira 后先重建完整预览再请求确认。Summary、Root Cause、Fix、Files Changed 和 Tests 均需完整命名真实对象；空行内代码、空路径/对象、截断值或缺失模板行必须使整份预览失效并重新渲染。`Git Delivery: auto` 还必须显式显示 `Commit Content Confirmation: PENDING`；选择 auto、启用 automation 或得到 `AUTO_COMMIT_AND_PUSH` 都不构成内容确认。预览末尾动态输出 `Action: CONFIRM_COMMIT`、`Owner: User`、`UI Route: CURRENT_INPUT`、`Dispatch Target: NONE`、`Input Required: YES`，在 `Required Input` 中逐项列出 Git Delivery、Jira ID、Decision 和 auto fingerprint，在 `Reply Template` 中给出完整可复制表单，并由 `Instruction` 明确要求直接在当前输入框回复、不要点击下一步。Decision 接受 `确认修改并提交`、auto 的 `确认自动提交内容` 或 `调整修改: <要求>`。用户最终确认后，当前 EmbeddedDeveloper 直接继续，不得自我委派或等待另一个按钮。预检重新运行相同路径的 `git-plan` 并比较 fingerprint；路径或内容漂移时确认失效并返回预览。用户未确认时返回 `BLOCKED` 且不执行 Git 写入。
...
- `COMMIT`：只使用已确认的 metadata 从仓库模板生成消息，用 `project_policy.py message` 校验后执行一个范围内 commit；Agent 参与时如实填写 AI 字段。不 amend 既有提交，不用 `--no-verify`，失败后不得继续 push。`auto` 还必须持有本轮匹配的 `content_confirmation.status: CONFIRMED`，不得把选择模式或按钮点击当作确认。`commit` 模式成功后进入 `REPORT`；`commit-and-push` 保存完整 SHA 与首次 fingerprint 并进入 `CONFIRM_PUSH`；`auto` 成功后不暂停，直接进入 `PUSH_PREFLIGHT`。 消息文件必须与最后一次确认的 `## Commit Message Preview` 逐字节一致，否则确认失效并返回 `CONFIRM_DELIVERY`。
...
- `CONFIRM_DELIVERY`: show one complete confirmation preview. `Commit Content` lists every selected file with inclusion, Git state, complete path, added/deleted counts, and a truthful standalone summary, followed by excluded paths, fingerprint, mode, and `Change Confirmation: PENDING`. First show `Template Source`, `Template Load: PASS`, the ordered field list, and the full `project_policy.py message` command, exit code, and `Message Validation: PASS`; any missing item makes confirmation unreachable. When Jira is valid, immediately emit `## Commit Message Preview` and show every ordered line generated from `.project/git/commit.template`, including multiline Test-Proposal steps, verbatim in one `text` fenced code block after `project_policy.py message` accepts it. Never substitute a Root Cause/Fix/Verification synopsis, `Generated Metadata`, table, or ellipsis; the message eventually passed to `git commit` is byte-for-byte identical to the displayed preview. If Jira is missing, show `BLOCKED - Jira ID required`, then regenerate the complete preview before asking for confirmation. Reject and rerender an empty inline-code span, empty path/object, truncated value, missing template line, or incomplete Summary, Root Cause, Fix, Files Changed, or Tests value. End with `Action: CONFIRM_COMMIT`, `Owner: User`, `UI Route: CURRENT_INPUT`, `Dispatch Target: NONE`, and `Input Required: YES`. `Required Input` itemizes Git Delivery, Jira ID, Decision, and the auto fingerprint; `Reply Template` supplies a complete copy-ready form; and `Instruction` explicitly says to reply in the current input without clicking Next Action. For `Git Delivery: auto`, also show `Commit Content Confirmation: PENDING`; selecting auto, enabling automation, or receiving `AUTO_COMMIT_AND_PUSH` is not content confirmation. Decision accepts `confirm changes and commit`, auto's `confirm automatic commit content`, or `adjust changes: <request>`. Drift invalidates confirmation and regenerates the preview. Without explicit confirmation, return `BLOCKED` and perform no Git write.
...
- `COMMIT`: generate the message from the repository template using only confirmed metadata and validate it with `project_policy.py message`; disclose agent participation in the AI fields. Create one in-scope commit, never amend or use `--no-verify`, and never proceed after commit failure. Auto additionally requires the matching `content_confirmation.status: CONFIRMED`; neither mode selection nor a button click is confirmation. On success, `commit` enters `REPORT`; `commit-and-push` preserves the full SHA and first fingerprint and enters `CONFIRM_PUSH`; `auto` proceeds directly to `PUSH_PREFLIGHT` without pausing. The message file is byte-for-byte identical to the last confirmed `## Commit Message Preview`; otherwise confirmation is invalid and control returns to `CONFIRM_DELIVERY`.
