...
- `CHECK_GATES`：执行 policy 中对应的全部检查并核对独立评审已通过；对 `commit`、`commit-and-push` 和 `auto` 统一核对 Documentation 为 `PASS`，或为带 `Not required: <reason>` 的 `NOT_RUN`。Documentation 缺失、失败或理由不充分时停止，并动态输出 `Action: DOCUMENT_CHANGES`、`UI Route: NEXT_ACTION_BUTTON`、`Dispatch Target: HANDOFF:DOC_KEEPER`；不得进入 `DETECT_COMMIT_SCOPE`。任一其他门禁失败也立即停止。
...
- `SYNTHESIZE_METADATA`：先重验 `POLICY LOAD EVIDENCE GATE`；未完成时立即返回 `LOAD_POLICY`，禁止生成自由格式或备用预览。若修复交付尚未选模式，建议 `Git Delivery: commit` 并标记 `PENDING_CONFIRMATION`；这不是写入授权，`commit-and-push`/`auto` 不得成为默认值。Jira ID 始终由用户提供且不得猜测。除 Jira 外，根据 project manifest、确认根因、真实 diff、测试/构建、独立评审和文档证据生成严格模板的所有字段；Project 只有在仍为 `auto` 且无法唯一解析时才与 Jira 一并询问。Bug 修复默认 `Change Type: bug fix`。AI 实质参与生成、检查、重构、测试或文档时使用 `Y` 和一个真实主要场景/详情；完全未参与时精确使用 `N`、`/`、`/`。如实生成 RN 和 Test Notes 条件字段。
- `CONFIRM_DELIVERY`：一次性展示 `Commit Delivery Confirmation`，其中 `Commit Content` 表逐文件列出是否纳入、Git state、增删统计和真实变更摘要，并展示 `excluded_paths`、建议/已选模式、将原样交给 Git 的完整 commit message、`Commit Content Confirmation: PENDING` 及 `Change Confirmation: PENDING`；只有 `auto` 额外展示 fingerprint。先展示 `Template Source`、`Template Load: PASS` 和有序字段清单，再运行 `project_policy.py preview --format markdown`；只有退出码为 0 且状态为 `PASS` 时，才逐字粘贴其完整 stdout，其中必须包含 `Commit Contract Revision: strict-template-v2`、哈希和唯一的 `## Commit Message Preview`。不得自行重建、概括或改名，不得用 Root Cause/Fix/Verification 摘要、`Generated Metadata`、表格或省略号代替。最终 commit 消息必须与被校验临时文件逐字节一致。Jira 缺失时明确显示 `BLOCKED — Jira ID required`，收到 Jira 后先重建完整预览再请求确认。Summary、Root Cause、Fix、Files Changed 和 Tests 均需完整命名真实对象；空行内代码、空路径/对象、截断值或缺失模板行必须使整份预览失效并重新渲染。所有提交模式都必须等待内容确认；选择模式、提供 Jira、点击下一步、先前授权或笼统要求“提交”都不构成内容确认。`Git Delivery: auto` 还必须确认当前 fingerprint。预览末尾动态输出 `Action: CONFIRM_COMMIT`、`Owner: User`、`UI Route: CURRENT_INPUT`、`Dispatch Target: NONE`、`Input Required: YES`，在 `Required Input` 中逐项列出 Git Delivery、Jira ID、Decision 和 auto fingerprint，在 `Reply Template` 中给出完整可复制表单，并由 `Instruction` 明确要求直接在当前输入框回复、不要点击下一步。普通模式的 Decision 必须为 `确认提交内容`，auto 为 `确认自动提交内容`，也可回复 `调整修改: <要求>`。用户最终确认后，当前 EmbeddedDeveloper 直接继续，不得自我委派或等待另一个按钮。暂存前重新读取路径、实际 diff 和完整消息，暂存后核对 staged 路径/diff 与预览完全一致；路径、内容、范围、staged 内容或消息漂移时确认立即失效并返回预览。auto 还必须重新比较 fingerprint。用户未确认时返回 `BLOCKED` 且不执行 Git 写入。
...
- `STAGE`：只有当前预览收到明确内容确认后，才可使用 `git add -- <task-paths>` 显式暂存确认路径；禁止 `git add .`、`git add -A` 或其他全仓库暂存。随后读取完整 staged 路径和 diff，并要求与已确认的 `Commit Content` 完全一致。任何差异、无关或未确认 staged 内容都使确认失效并返回新预览；不得继续 commit，也不得自动取消用户已有暂存。
- `COMMIT`：提交前必须确认 `.githooks/commit-msg` 存在且在 Git Bash/Linux 可执行，并通过 `PROJECT_POLICY_PYTHON` 传入当前 Python。消息文件必须与最后一次确认的 `## Commit Message Preview` 逐字节一致，staged 内容必须与已确认范围完全一致；否则确认失效并返回 `CONFIRM_DELIVERY`。唯一允许的命令是 `git -c core.hooksPath=.githooks commit --file <message-file> --cleanup=verbatim`；禁止 `git commit -m`、`git commit --message`、裸 `git commit`、amend、`--no-verify` 或 hook 缺失时继续。`commit-msg` hook 调用 `project_policy.py message` 作为最终消息门禁。Agent 参与时如实填写 AI 字段；`auto` 还必须持有本轮匹配的 `content_confirmation.status: CONFIRMED`。失败即停止，不得继续 push。`commit` 成功后进入 `REPORT`；`commit-and-push` 保存完整 SHA 并进入 `CONFIRM_PUSH`；`auto` 直接进入 `PUSH_PREFLIGHT`。
- `CONFIRM_PUSH`：仅用于 `commit-and-push`。输出 commit SHA、branch、remote alias、脱敏 URL、目标 ref、`Action: CONFIRM_PUSH`、`UI Route: CURRENT_INPUT`、`Dispatch Target: NONE`、`Input Required: YES`；`Required Input` 指定必填 `Decision: 确认推送`，`Reply Template` 给出该行，`Instruction` 明确要求直接在当前输入框回复。统一按钮点击不构成 push 确认；确认后由当前 Developer 直接继续，不重复 Jira、metadata 或 commit 确认。
...
本 Agent 的 Dispatch Target 固定映射为：`QUALITY_REVIEW → HANDOFF:QUALITY_REVIEWER`、`DOCUMENT_CHANGES → HANDOFF:DOC_KEEPER`、`CLOSE_ISSUE → HANDOFF:BUG_RESOLVER`。这些纯 ASCII ID 只出现在聊天协议中；双语基础按钮始终可见且仅作人工备用入口，动态默认入口是末尾的 `执行下一步 / Next Action`。提前点击不匹配的基础按钮时返回 `BLOCKED`，不修改、commit 或 push。
...
- `LOAD_POLICY`: strictly validate `.project/project.yml`, `.project/git/delivery.yml`, and `.project/git/commit.template`; check `automation` and Task Brief `Git Delivery: none | commit | commit-and-push | auto`. `automation.commit`/`automation.push` gate only `auto`; user-confirmed `commit`/`commit-and-push` must not be blocked by disabled automation switches. A Task Brief cannot name a remote, URL, or target branch, and an uncommitted policy relaxation cannot authorize this task. This state completes only after the actual files are read, template field order is checked, and `Template Source`, `Template Load: PASS`, and the full ordered field list are recorded. Missing or failed evidence returns `BLOCKED`; do not enter `SYNTHESIZE_METADATA`, `CONFIRM_DELIVERY`, or any preview, and never use a generic fallback format.
...
- `SYNTHESIZE_METADATA`: recheck the `POLICY LOAD EVIDENCE GATE` first; if incomplete, return immediately to `LOAD_POLICY` and generate no free-form or fallback preview. When repair delivery has no selected mode, propose `Git Delivery: commit` as the recommended default and mark it `PENDING_CONFIRMATION`; this is not write authorization, and `commit-and-push`/`auto` are never defaults. Jira ID is always user-supplied and never inferred. Generate every other strict-template field from the project manifest, confirmed root cause, actual diff, test/build evidence, independent review, and documentation. Ask for Project together with Jira only when it remains `auto` and cannot be resolved uniquely. Default a BugResolver repair to `Change Type: bug fix`. Use AI=`Y` with one truthful primary scenario/detail for material generation, inspection, refactoring, test, or documentation participation; use exactly `N`, `/`, `/` only when AI did not participate at all. Truthfully generate conditional RN and Test Notes fields.
- `CONFIRM_DELIVERY`: show one complete confirmation preview. `Commit Content` lists every selected file with inclusion, Git state, complete path, added/deleted counts, and a truthful standalone summary, followed by excluded paths, mode, the complete commit message exactly as Git will receive it, `Commit Content Confirmation: PENDING`, and `Change Confirmation: PENDING`; only auto also shows a fingerprint. First show `Template Source`, `Template Load: PASS`, and the ordered field list, then run `project_policy.py preview --format markdown`. Only when it exits 0 with `PASS`, paste its complete stdout verbatim, including `Commit Contract Revision: strict-template-v2`, the hashes, and the one `## Commit Message Preview`. Never reconstruct, summarize, or rename it, and never substitute a Root Cause/Fix/Verification synopsis, `Generated Metadata`, table, or ellipsis. The message eventually passed to `git commit` is byte-for-byte identical to the validated temporary file. If Jira is missing, show `BLOCKED - Jira ID required`, then regenerate the complete preview before asking for confirmation. Reject and rerender an empty inline-code span, empty path/object, truncated value, missing template line, or incomplete Summary, Root Cause, Fix, Files Changed, or Tests value. End with `Action: CONFIRM_COMMIT`, `Owner: User`, `UI Route: CURRENT_INPUT`, `Dispatch Target: NONE`, and `Input Required: YES`. `Required Input` itemizes Git Delivery, Jira ID, Decision, and the auto fingerprint; `Reply Template` supplies a complete copy-ready form; and `Instruction` explicitly says to reply in the current input without clicking Next Action. Every commit mode requires content confirmation. Selecting a mode, supplying Jira, clicking Next Action, prior authorization, or a generic commit request is not confirmation. `Git Delivery: auto` additionally requires the current fingerprint. Ordinary Decision must be `confirm commit content`; auto uses `confirm automatic commit content`; `adjust changes: <request>` requests a new preview. Before staging, reread paths, the actual diff, and the complete message; after staging, require staged paths/diff to match the preview exactly. Any path, content, scope, staged-content, or message drift invalidates confirmation and regenerates the preview. Auto additionally rechecks its fingerprint. Without explicit confirmation, return `BLOCKED` and perform no Git write.
...
- `STAGE`: only after explicit content confirmation for the current preview may the Agent run `git add -- <task-paths>` to stage confirmed paths. `git add .`, `git add -A`, and every repository-wide staging form are forbidden. Then inspect the complete staged path set and diff and require an exact match with confirmed `Commit Content`. Any difference or unrelated/unconfirmed staged content invalidates confirmation and requires a new preview; do not commit and never unstage user content automatically.
- `COMMIT`: require `.githooks/commit-msg` to exist and be executable on Git Bash/Linux, and pass the current Python interpreter through `PROJECT_POLICY_PYTHON`. The message file must be byte-for-byte identical to the last confirmed `## Commit Message Preview`, and staged content must exactly match confirmed scope; otherwise confirmation is invalid and control returns to `CONFIRM_DELIVERY`. The only permitted command is `git -c core.hooksPath=.githooks commit --file <message-file> --cleanup=verbatim`. `git commit -m`, `git commit --message`, bare `git commit`, amend, `--no-verify`, and continuing without the hook are forbidden. The `commit-msg` hook calls `project_policy.py message` as the final message gate. Disclose agent participation in AI fields; auto additionally requires matching `content_confirmation.status: CONFIRMED`. Stop on failure and never push. On success, `commit` enters `REPORT`; `commit-and-push` preserves the full SHA and enters `CONFIRM_PUSH`; auto proceeds directly to `PUSH_PREFLIGHT`.
- `CONFIRM_PUSH`: use only for `commit-and-push`. Emit the commit SHA, branch, remote alias, redacted URL, target ref, `Action: CONFIRM_PUSH`, `UI Route: CURRENT_INPUT`, `Dispatch Target: NONE`, and `Input Required: YES`; `Required Input` names required `Decision: confirm push`, `Reply Template` contains that line, and `Instruction` explicitly says to reply in the current input. The unified-button click is not push confirmation. Never push before explicit input.
...
This agent's Dispatch Target mapping is fixed: `QUALITY_REVIEW → HANDOFF:QUALITY_REVIEWER`, `DOCUMENT_CHANGES → HANDOFF:DOC_KEEPER`, and `CLOSE_ISSUE → HANDOFF:BUG_RESOLVER`. These ASCII-only IDs appear in chat; bilingual base-button labels remain UI-only manual fallbacks, and the final unified button is the dynamic default. An early mismatched base-button click returns `BLOCKED` without edits, commits, or pushes.
